<?php
	...
	header("Content-Disposition: attachment; filename=laporan_peminjaman_alat.xls");
...
?><center>
   ...
   <table border="1">
        ...
        <tbody>
            <?php
                $no =1;
                $show = mysql_query("SELECT * from trx_loan_application A join trx_loan_application_detail B on A.loan_app_id = B.loan_app_id_fk join tbl_member C on C.member_id = A.member_id_fk join ref_instrument D on D.instrument_id = B.instrument_id_fk WHERE A.loan_date_start BETWEEN '$per1' AND '$per2' AND A.loan_status='DIPINJAM' ");
